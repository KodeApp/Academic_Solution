...
public class StudentAttendance<StudentItems> extends AppCompatActivity {
    ...
    private void saveStatus() {
        for (StudentItem studentItems : studentItems){
           String status = studentItems.getStatus();
           if (status!="P") status = "A";
           long value = dbHelper.addStatus(studentItems.getSid(),cid,calendar.getDate(),status);

           if (value==-1)dbHelper.updateStatus(studentItems.getSid(),calendar.getDate(),status);
        }
    ...
    private void openSheetList() {
        ...
        intent.putExtra("rollArry",rollArray);
