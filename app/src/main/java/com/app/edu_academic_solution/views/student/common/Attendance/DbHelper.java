...
//    }
    long addStatus(long sid, long cid, String date, String status) {
        ...
        Cursor cursor = database.query(
                ...
                null,
                S_ID + " = ? AND " + DATE_KEY + " = ?",
                new String[]{String.valueOf(sid), date},
                null,
                null,
                null
        );
        ...
        if (cursor != null && cursor.moveToFirst()) {
            // Row already exists, update the status instead
            ContentValues values = new ContentValues();
            values.put(STATUS_KEY, status);
            long updatedRows = database.update(
                    STATUS_TABLE_NAME,
                    values,
                    S_ID + " = ? AND " + DATE_KEY + " = ?",
                    new String[]{String.valueOf(sid), date}
            ...
            values.put(DATE_KEY, date);
            values.put(STATUS_KEY, status);
            long insertedRowId = database.insert(STATUS_TABLE_NAME, null, values);
            cursor.close();
